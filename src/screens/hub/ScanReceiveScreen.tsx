...
import { getBoxByCode, updateBoxStatus, logInventoryEvent } from '@/lib/supabase';
...
export default function ScanReceiveScreen({ navigation }: any) {
  ...
  const handleScan = async (data: string, parsed: QRPayload | null) => {
    ...
    setLoading(true);
    try {
      const code = parsed?.box_id ?? data;
      const { data: boxData, error } = parsed?.box_id
        ? await import('@/lib/supabase').then(m => m.getBoxById(code))
    ...
    } catch (e) { Alert.alert('Error', 'Failed to fetch box details.'); }
    setLoading(false);
  };
  ...
  const handleReceive = async () => {
    ...
    setLoading(true);
    try {
      const { error } = await updateBoxStatus(box.id, 'received');
  ...
  if (scanning) {
    return (
      <View style={{ flex: 1 }}>
        <ScannerCamera onScan={handleScan} onClose={() => setScanning(false)} />
      </View>
    );
  }
          ...
          {done ? (
            ...
            <>
              <View style={styles.boxHeader}>
                ...
                <Badge
                  label={box.status.replace('_', ' ')}
                  color={statusColor[box.status] ?? 'gray'}
                />
              </View>
...
const styles = StyleSheet.create({
  ...
  scanBorder: {
    height: 240, borderRadius: Radii.xl,
    borderWidth: 2, borderColor: Colors.primaryBorder, borderStyle: 'dashed',
    alignItems: 'center', justifyContent: 'center', gap: 10,
    backgroundColor: Colors.primaryBg, position: 'relative',
  },
  ...
  btnQC: {
    ...
    borderWidth: 1.5, borderColor: Colors.primaryBorder, backgroundColor: Colors.primaryBg,
  },
